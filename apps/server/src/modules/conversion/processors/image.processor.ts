...
import { IMAGE_QUEUE } from '../../../queue/queue.module';
...
@Processor(IMAGE_QUEUE, { concurrency: 5 })
export class ImageProcessor extends WorkerHost {
  ...
  async process(job: Job<ConvertJobData>): Promise<void> {
    ...
    try {
      ...
      // 确定输出目录
      const outputDir = path.resolve(process.env.UPLOAD_DIR || './uploads', 'output', 'image');

      // 执行转换
      const result = await this.imageStrategy.convert({
      ...
      this.logger.log(`图片任务完成: ${taskNo}`);
    } catch (err) {
      this.logger.error(`图片任务失败: ${taskNo}`, err instanceof Error ? err.stack : err);
  ...
  onFailed(job: Job<ConvertJobData> | undefined, err: Error) {
    if (job) {
      this.logger.error(`图片 Worker 失败: ${job.data.taskNo}`, err.stack);
    }
  }
}
